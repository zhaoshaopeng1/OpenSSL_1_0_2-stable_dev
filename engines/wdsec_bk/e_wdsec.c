	...
	struct wd_queue *q)
{
	...
	cipher_ctx->msg.aflags |= WD_AATTR_FLAT;
	...
	const unsigned char *key, const unsigned char *iv, int enc)
{
	...
	//
		...
		//memcpy(g_iv, iv, EVP_CIPHER_CTX_iv_length(ctx));
	#endif
	return 1;
}
	...
	unsigned char *out, const unsigned char *in, size_t in_len)
{
	...
	dst = (void *)cipher_ctx->dma_map.vaddr + in_len;
	memset(dst, 0, in_len);
	key = (void *)dst + in_len;
